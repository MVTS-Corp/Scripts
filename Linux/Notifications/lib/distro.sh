...
#
# PURPOSE:
# Detects the host Linux distribution family and its package manager, so
# install.sh knows how to install msmtp. Sourced, not executed. Requires
# common.sh (for die) to already be sourced. Deliberately a standalone
# copy rather than one shared with Linux/Updates; see common.sh for why.
